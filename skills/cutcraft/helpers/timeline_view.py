...
def extract_frames(video: Path, start: float, end: float, n: int, dest_dir: Path) -> list[Path]:
    ...
    if n < 1:
        n = 1
    # An exact EOF seek has no decodable frame on some MP4s.  Keep the last
    # sample infinitesimally inside the requested range so contact sheets work
    # for a full-duration inspection as well as a trimmed interval.
    sample_end = max(start, end - 0.001)
    if n == 1:
        times = [(start + sample_end) / 2.0]
    else:
        step = (sample_end - start) / (n - 1)
